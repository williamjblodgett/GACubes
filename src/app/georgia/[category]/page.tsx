import { Metadata } from "next";
import Link from "next/link";
import { notFound } from "next/navigation";
import { GA_CATEGORIES, GA_CITIES } from "@/lib/seed-data";
import { CATEGORY_LABELS, CATEGORY_COLORS } from "@/lib/types";
import { getListingsByCategory } from "@/lib/search";
import { MapPin, ArrowRight } from "lucide-react";
import Header from "@/components/Header";

interface Props { params: Promise<{ category: string }> }

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { category: slug } = await params;
  const cat = GA_CATEGORIES.find((c) => c.slug === slug);
  if (!cat) return { title: "Not Found" };
  return { title: `${cat.label} in Georgia - GACubes`, description: `Find ${cat.label.toLowerCase()} across Georgia.` };
}

export function generateStaticParams() { return GA_CATEGORIES.map((c) => ({ category: c.slug })); }

export default async function CategoryPage({ params }: Props) {
  const { category: slug } = await params;
  const cat = GA_CATEGORIES.find((c) => c.slug === slug);
  if (!cat) notFound();
  const listings = getListingsByCategory(cat.category);

  return (
    <div className="min-h-screen bg-surface-secondary">
      <Header />
      <main className="max-w-5xl mx-auto px-4 py-8">
        <div className="flex items-center gap-2 text-sm text-muted mb-4">
          <Link href="/" className="hover:text-primary">Home</Link><span>/</span><span className="text-foreground">{cat.label}</span>
        </div>
        <h1 className="text-3xl font-bold text-foreground mb-2">{cat.label} in Georgia</h1>
        <p className="text-muted mb-8">Find {cat.label.toLowerCase()} locations across Georgia.</p>

        {listings.length === 0 ? (
          <div className="bg-surface rounded-xl border border-border p-8 text-center">
            <p className="text-muted">No listings found yet.</p>
            <Link href="/submit" className="inline-flex items-center gap-1 mt-3 text-primary text-sm hover:underline">Submit a location <ArrowRight size={14} /></Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            {listings.map((l) => (
              <Link key={l.id} href={`/listing/${l.id}`} className="bg-surface rounded-xl border border-border p-5 hover:shadow-md transition-shadow">
                <h2 className="font-semibold text-foreground">{l.name}</h2>
                <div className="flex items-center gap-1 text-sm text-muted mt-1"><MapPin size={14} />{l.address}, {l.city}, GA</div>
                <div className="flex flex-wrap gap-1 mt-3">
                  <span className={`px-2 py-0.5 rounded text-xs font-medium text-white ${CATEGORY_COLORS[l.category_primary]}`}>{CATEGORY_LABELS[l.category_primary]}</span>
                  {l.open_24h && <span className="px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300">24/7</span>}
                </div>
              </Link>
            ))}
          </div>
        )}

        <div className="mt-8">
          <h2 className="text-xl font-bold text-foreground mb-4">{cat.label} by City</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {GA_CITIES.slice(0, 16).map((city) => (
              <Link key={city.slug} href={`/city/${city.slug}`} className="flex items-center gap-2 p-3 bg-surface rounded-lg border border-border hover:border-primary text-sm text-foreground font-medium">
                <MapPin size={14} className="text-primary shrink-0" />{city.name}
              </Link>
            ))}
          </div>
        </div>
        <div className="mt-8">
          <Link href="/search" className="inline-flex items-center gap-2 px-5 py-2.5 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary-hover">Search all categories <ArrowRight size={16} /></Link>
        </div>
      </main>
    </div>
  );
}
